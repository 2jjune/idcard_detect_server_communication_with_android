...
def main():
    ...
    frames = []
    imagePath = 'C:/Users/USER/PycharmProjects/homography/segment_result/videos_frame_4_train/'
    for file in video_list:
        ...
        while True:
            ...
            if not ret:
                break
            if count % 6 == 0:
                cv2.imwrite(imagePath + "/{0}_frame{1}.jpg".format(file[:-4],count), image)
                frames.append(image)
            count += 1
        print(len(frames))
        cap.release()
        ...
        #         continue
        #     else:
        #         cv2.imwrite('C:/Users/USER/PycharmProjects/homography/segment_result/demo_frame/{0}_{1}.jpg'.format(file[:-4],check_num), receipt_image)
        #         check_num += 1
