...
img_height = 512
img_width = 512
dir = 'C:\\Users\\USER\\PycharmProjects\\img_dataset_for_edge_detection\\imgs\\train'

base_dir = 'C:/Users/USER/PycharmProjects/img_dataset_for_edge_detection/'
...
#         img_file = cv2.imread(train_dir+i)
#         img_file = cv2.resize(img_file, (img_height, img_width))
#         label_file = cv2.imread(train_label_dir+i)
#         label_file = cv2.resize(label_file, (img_height, img_width))
#
#
...
#             tmp+=1
#         else:
#             # print(img_file_list[0][0], label_file_list[0][0])
#             yield(img_file_list,label_file_list)
#             img_file_list=[]
#             # print('1'*100)
#             label_file_list=[]
#             tmp=0
...
#     img_file = cv2.imread(val_dir + i)
#     img_file = cv2.resize(img_file, (img_height, img_width))
#     label_file = cv2.imread(val_label_dir + i)
#     label_file = cv2.resize(label_file, (img_height, img_width))
#
#     img_file = np.asarray(img_file)
#     label_file = np.asarray(label_file)
...
#     label_file = label_file / 255.
#
#     val_img_file.append(img_file)
#     val_label_file.append(label_file)
...
# callback = [tf.keras.callbacks.ModelCheckpoint(filepath='./checkpoint/best.h5', save_weights_only=True, save_best_only=True),
...
# history = model.fit(train_data, epochs=50, batch_size=batch_size, shuffle=True, validation_data=(val_img_file, val_label_file), callbacks=callbacks)
