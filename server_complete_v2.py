...
def warp_classification(img, imagefile):
    global current_t
    ###############원래 있던부분
...
def handle_request():
    ...
    result = warp_classification(img, imagefile)
...
if __name__ == '__main__':
    ...
    # th1 = Thread(target = timeout, args=(current_t))
    # th2 = Thread(target = app.run(host="0.0.0.0", port=port, debug=True))
    # th2.daemon = True
    #
    # th1.start()
    # th2.start()
